...
namespace nonstd {
  ...
  private:
    ...
    static constexpr inline INTEGER_TYPE make_min_int() {
      INTEGER_TYPE min_representable = -pow10_128(PRECISION) < (int64_t) std::numeric_limits<INTEGER_TYPE>::min()
                                       ? std::numeric_limits<INTEGER_TYPE>::min() + 1 : -pow10_128(PRECISION);
    ...
    constexpr static inline const INTEGER_TYPE make_max_int() {
      return pow10_128(PRECISION) > std::numeric_limits<INTEGER_TYPE>::max()
             ? std::numeric_limits<INTEGER_TYPE>::max() : pow10_128(PRECISION);
    }
  ...
  public:
    ...
    template<typename INTEGER_TYPE2, unsigned int PRECISION2, unsigned int SCALE2>
  ...
  operator*(const decimal<INTEGER_TYPE, PRECISION, SCALE> &c1,
            const decimal<INTEGER_TYPE, PRECISION, SCALE> &c2) {
    return decimal<INTEGER_TYPE, PRECISION, SCALE>::make_from_rep(((int128_t) c1.get_rep() *
                                                                  c2.get_rep()) /  pow10_128(SCALE));
  }
...
// math functions
  ...
  // catch all types that is inefficient but useful for generic conversion
  class decimal32{
  public:
    decimal32(unsigned int PRECISION, unsigned int SCALE, double v) : precision_(PRECISION), scale_(SCALE), value_(0.5 + v * pow10_128(SCALE)) {}
  ...
  class decimal64{
  public:
    decimal64(unsigned int PRECISION, unsigned int SCALE, double v) : precision_(PRECISION), scale_(SCALE), value_(0.5 + v * pow10_128(SCALE)){
    }
    const unsigned int precision_;
    const unsigned int scale_;
    int64_t value_;
